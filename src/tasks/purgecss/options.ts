 ...
 */
export type PurgecssOptions = Omit<Partial<PurgecssDefaults>, 'content' | 'css'> &
  Pick<PurgecssDefaults, 'content' | 'css'>
  ...
  schema: (joi) => {
    return joi.object({
      writeRejected: joi
        .boolean()
        .description(
          'Write a text file under `/public` a list of rejected selectors for every file.',
        ),
      ignoreAssets: joi
        .object({
          webpack: joi
          ...
          css: joi.array().items(joi.string()).description('CSS files to exclude from processing.'),
          js: joi
            .array()
            .items(joi.string())
        ...
        .description(
          'CSS/JavaScript files to ignore during optimization. File names should be relative to `/public` directory',
        ),
      allowSymbols: joi
        .boolean()
        .description(
      ...
      fontFace: joi.boolean().description('Remove any unused `@font-face` rules in your css.'),
      keyframes: joi.boolean().description('Remove any unused `@keyframes` rules in your css.'),
      variables: joi.boolean().description('Remove any unused variables in your css.'),
    })
  },
}
